...
RSpec.describe Api::V1::FeedsController, type: :controller do
  ...
  describe "GET#index" do
    it "returns all melodies and current_user and current_user_votes" do
      scale1 = Scale.create(name: "A Minor", notes: "D2 E2 F2 G2 A2 B2 C3 D3 E3 F3 G3 A3 B3 C4 D4 E4 F4 G4 A4 B4 C5 D5 E5 F5")
      stock1 =
        Stock.create(
          symbol: "IBM",
      ...
      melody1 = Melody.create(user: user1, scale: scale1, stock: stock1)
      like1 = Like.create(user: user1, melody: melody1)
      ...
      expect(response_body["melodies"]["melodies"][0]["likes"][0]["id"]).to eq melody1.likes[0].id
      expect(response_body["melodies"]["melodies"][0]["get_name"]).to eq melody1.get_name

      expect(response_body["currentUser"]["id"]).to eq user1.id

      expect(response_body["currentUserLikes"][0]["id"]).to eq like1.id
    end
  end
end
